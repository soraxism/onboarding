...
| 値 | ラベル案 | 付くタイミング |
|---|---|---|
| `goal_started`（**既定**・現行挙動） | ゴールを開いたとき | 1ステップ目の表示時（`display_goals` 記録時） |
| `last_step_displayed` | 最後のステップを表示したとき | 最終ステップの表示時（レポートの完了集計と同一基準） |
...
レポートの完了定義（最終ステップの表示）と `last_step_displayed` は一致するため、
「完了」という語を使っても数値との矛盾は生じない。
ただし選択肢の内容がそのまま「いつチェックが付くか」なので、
設定項目名は **「イントロのチェックマークを付けるタイミング」** とし、
選択肢ラベルで動作（ゴールを開いたとき / 最後のステップを表示したとき）を説明する形が明快。
