...
| ツアー | 内容 | 状態 |
|---|---|---|
| `23` | `onb_ext_start` で `onb_display_goals_` に直接ゴールIDを書き足し、特定ゴールを強制的に表示済み扱いにする | **記述が残っているだけ**（対象ゴールは利用停止・2026-09-14 担当者確認） |
| `957` | イントロを開くたびに `onb_display_goals_` 基準で「確認済み / 未確認」を自前描画 | **公開停止中** |

削除済み: `982`・`999`（いずれも `957` と同型）。

**現時点で実害のあるものは無い。** ただし `957` を再開する場合、または `23` の
`freeway-sample` を再び使う場合は、移行時にカスタムJS側の描画も併せて直す必要がある。
`23` は「強制的にチェックを付ける」意図そのものが `last_step_displayed` では成立しない
