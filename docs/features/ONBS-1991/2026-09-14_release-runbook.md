...
## 事前確認（2026-09-14 時点で確認済み）

- **`release` ブランチは 4 リポジトリとも base と同一**（`origin/{base}..origin/release` が 0 コミット）。
  他者の実装は混じっていない
- 5 PR とも **mergeable / CLEAN、CodeRabbit 指摘ゼロ**
- チェックリスト 51 項目 + 再確認 3 項目が OK

### リリース前に終わらせること

| # | 内容 | 状態 |
|---|---|---|
| §7-6 | 本番の顧客カスタム JS を持つツアー（`13` と `23`）での確認 | **未実施** |
| §10 | レポート整合（日次バッチのため翌日確認） | 仕込み済み・**確認待ち** |
...
| # | 内容 |
|---|---|
| 1 | 管理画面のツアー「表示スタイル設定」→ イントロに「チェックマークを付けるタイミング」が出る |
| 2 | 既定は「ゴールの表示」。**既存ツアーの見え方が変わっていない** |
| 3 | エディタ拡張でチェック色だけ変更 → 管理画面でタイミングが残っている（①の部分更新が効いている） |
| 4 | 顧客カスタム JS を持つツアー（`13`・`23`）で従来どおりの表示・コンソールエラーなし |

**2 と 4 がいちばん重要。** この対応は「既定では何も変わらない」ことが前提になっている。
